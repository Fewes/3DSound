...
int main()
{
	...
	*/
	...
	as[0] << sound.getChannel(0); //Asign left sound to the left (audiostreamname[0]) buffer in audiostream 
	as[1] << sound.getChannel(1); //Asign right sound to the left (audiostreamname[1]) buffer in audiostream

	AudioPlayback pb(sound.getSampleRate(), 2, 16); //Make a playback "source" that plays two channels and has right samplerate and 16bit/sample
